...
class Solution:
    def compareVersion(self, version1, version2):
        # first we want to replace the . and 0 in the strings so that we
        # can compare them by turning them into integers

        # if int(version1) > int(version2) then version1 is higher
        if int(version1.replace('.', '').replace('0', '')) - int(version2.replace('.', '').replace('0', '')) > 0:
            return 1

        # if int(version2) > int(version1) then version2 is higher
        if int(version2.replace('.', '').replace('0', '')) - int(version1.replace('.', '').replace('0', '')) > 0:
            return -1
